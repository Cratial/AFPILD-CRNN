...
class BaseTrainer:
    ...
    def __init__(self, torch_args: dict, save_dir, **kwargs):
        ...
        if self.early_stop <= 0 or self.early_stop is None:  # early stop for training
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs + 1):
            ...
            # update the best score of person identification (accuracy) and localization (theta error)
            current_acc = log_mean['val_accuracy_l']
            current_acc30 = log_mean['val_accuracy_l30']
            current_loc = log_mean['val_mae_d']
            if current_acc > best_val_acc:
                best_val_acc = current_acc
                print('best val acc updated with {}'.format(best_val_acc))

            if current_acc30 > best_val_acc30:
                best_val_acc30 = current_acc30
                print('best val acc (threshold 30) updated with {}'.format(best_val_acc30))

            if current_loc < best_val_loc:
                best_val_loc = current_loc
                print('best val loc updated with {}'.format(best_val_loc))
            ...
            if self.mnt_mode != 'off':
                ...
                # a number of not improved training trigger the early stop
                if not_improved_count > self.early_stop:
                    self.logger.info("Validation performance didn\'t improve for {} epochs. "
                                     "Training stops.".format(self.early_stop))
                    print('best val acc is {}'.format(best_val_acc))
                    break
    ...
    def _save_checkpoint(self, epoch, save_best=False):
        ...
        if save_best:
            if self.save_the_best:
                filename = str(self.model_dir / f"model_best.pth")
            else:
                self.num_best += 1
                filename = str(self.model_dir / f"model_best{self.num_best}.pth")
        else:

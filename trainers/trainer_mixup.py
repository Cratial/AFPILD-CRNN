...
from utils import inf_loop, mixup_data_afpild
from utils import inf_loop
import copy
import wandb


class CRNNTrainerWithMixup(BaseTrainer):
    """
    Trainer class
    """
    ...
    def __init__(self, torch_args: dict, save_dir, resume, device, **kwargs):
        ...
        self.task = kwargs['task'].split('_')[0]
        ...
        self.accdoa_criterion = self.losses['accdoa_loss']
    ...
    def ideloc_mixup_criterion(self, pred, y_a, y_b, lam, ide_criterion_fn, loc_criterion_fn):
        ide_loss = lam * ide_criterion_fn(pred[:, :40], y_a[:, 0].type(torch.int64)) + (1 - lam) * ide_criterion_fn(pred[:, :40], y_b[:, 0].type(torch.int64))
        loc_loss = lam * loc_criterion_fn(pred[:, 40], y_a[:, 1]) + (1 - lam) * loc_criterion_fn(pred[:, 40], y_b[:, 1])
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, (data, target) in enumerate(self.train_data_loaders['data']):
            ...
            if self.mixup:
                if self.probability_mixup is not None:
                    if (1 - self.probability_mixup) > random.random():
                        data_mixup, targets_a, targets_b, lam = mixup_data_afpild(data, target, 0.2, True)
                        data = data_mixup
                        instance_mixup = 1
                    else:
                        instance_mixup = 0
            ...
            if self.mixup and instance_mixup:
                if self.task == 'ide':
                    loss = lam * self.ide_criterion(output, targets_a) + (1 - lam) * self.ide_criterion(output, targets_b)

                elif self.task == 'loc':
                    loss = lam * self.loc_criterion(output, targets_a) + (1 - lam) * self.loc_criterion(output, targets_b)

                elif self.task == 'ideloc':
                    loss = self.ideloc_mixup_criterion(output, targets_a, targets_b, lam, self.ide_criterion, self.loc_criterion)

                elif self.task == 'accil':
                    loss = lam * self.accdoa_criterion(output, targets_a) + (1 - lam) * self.accdoa_criterion(output, targets_b)

            else:
                if self.task == 'ide':
                    loss = self.ide_criterion(output, target)

                elif self.task == 'loc':
                    loss = self.loc_criterion(output, target)
                    ...
                    loss = self.accdoa_criterion(output, target)
    ...
    def _valid_epoch(self, epoch):
        ...
        # # add histogram of model parameters to the tensorbsoard

...
#                 modification = {opt.target: getattr(args, _get_opt_name(opt.flags)) for opt in options}
    ...
    def init_obj(self, keys, module, *args, **kwargs):
        ...
        try:
            ...
            module_obj = importlib.import_module(module_name, package=module)  # {module_name}.py
        except KeyError:  # In case no 'module' is specified
            module_obj = module
        class_name = obj_config['type']
        obj = getattr(module_obj, class_name)  # target object return
        kwargs_obj = self._update_kwargs(obj_config, kwargs)  # object related kwargs
        if 'base' not in class_name.lower() and 'optimizers' not in keys and 'losses' not in keys:
            kwargs_obj['task'] = self.run_args.task

        if obj_config.get('is_ftn', False):  # return as function
